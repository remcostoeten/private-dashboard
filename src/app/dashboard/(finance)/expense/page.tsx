...
const ExpensesPage = () => {
    ...
    const handleAddExpense = async (category: string, amount: number, date: Date, description: string) => {
        try {
            await createExpense(user.uid, category, amount, date, description)
            setExpenses(prevExpenses => [...prevExpenses, { category, amount, date, description }])
        } catch (err) {
            console.error(err)
